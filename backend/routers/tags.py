...
async def list_all_tags(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    tag_diary: dict[str, int] = {}
    tag_obj:   dict[str, int] = {}

    for entry in (await db.execute(select(DiaryEntry).where(DiaryEntry.user_id == current_user.id))).scalars().all():
        for t in (entry.tags or []):
            tag_diary[t] = tag_diary.get(t, 0) + 1

    for obj in (await db.execute(select(KnowledgeObject).where(KnowledgeObject.user_id == current_user.id))).scalars().all():
...
@router.put("/rename")
async def rename_tag(payload: TagRename, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def delete_tag(tag_name: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    for entry in (await db.execute(select(DiaryEntry).where(DiaryEntry.user_id == current_user.id))).scalars().all():
...
@router.get("/search")
async def search_tags(q: str = "", current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Return tags whose name starts with or contains q."""
    result = await list_all_tags(current_user, db)
