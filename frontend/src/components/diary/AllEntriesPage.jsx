...
export default function AllEntriesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      setEntries(await getAllEntries(activeTag))
  ...
  const handleSaved = (updated) => {
    ...
    if (old && old.date !== updated.date) { load(); setEditingId(null); setViewingId(null) }
    else { setEntries(prev => prev.map(e => e.id === updated.id ? updated : e)); setEditingId(null); setViewingId(updated.id) }
  }
    ...
    if (entry) return (
      <div className={styles.fullscreenEditor}>
        <div className={styles.editorTopBar}>
          <button className={styles.backBtn} onClick={() => { setEditingId(null); setViewingId(entry.id) }}>
            ← Back to entry
          </button>
          <span className={styles.editorDate}>{format(parseISO(entry.date), 'd MMMM yyyy')}</span>
        </div>
        <DiaryEditor
          entry={entry}
          onSave={handleSaved}
  ...
  if (viewingId) {
    ...
    if (entry) {
      ...
      const cleanTags = (entry.tags || []).filter(t => !t.startsWith('-') && !t.includes('--') && t.length < 40)
      return (
        <div className={styles.readPage}>
          ...
          <div className={styles.readCard}>
            <div className={styles.readMeta}>
              <span className={styles.readDate}>{format(parseISO(entry.date), 'EEEE, d MMMM yyyy')}</span>
            ...
            <div className={styles.readContent}>
              ...
              }) || <span className={styles.emptyNote}>Empty note — click Edit to write.</span>}
            </div>
            {cleanTags.length > 0 && (
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <span className={styles.count}>{loading ? '…' : `${entries.length} entries`}</span>
        {activeTag && (
          ...
          {dates.map(date => (
            <div key={date} className={styles.dateGroup}>
              <div className={styles.dateLabel}>
                <span className={styles.dateBadge}>{format(parseISO(date), 'EEEE, d MMMM yyyy')}</span>
              ...
              <div className={styles.cardGrid}>
                {grouped[date].map(entry => {
                  ...
                  const cleanTags = (entry.tags || []).filter(t => !t.startsWith('-') && !t.includes('--') && t.length < 40)
                  return (
                    <div key={entry.id} className={styles.card} onClick={() => setViewingId(entry.id)}>
